...
f_res = open('second_resAugmentedMI.txt', 'w')
a = f_freq.read().split()
tf = f_tf.read().split()
dict = {}
for i in range(len(a) / 4):
    dict[a[i * 4] + ' ' + a[i * 4 + 1] + ' ' + a[i * 4 + 2]] = math.log(1.0 * w * float(a[i * 4 + 3]) / ((float(tf[i * 4 + 2]) - float(a[i * 4 + 3]) + 1) * (float (tf[i * 4 + 3]) - float(a[i * 4 + 3]) + 1)))
